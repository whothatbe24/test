...
int main(int argc, char **argv)
{
    ...
    // 【订阅】vicon uav Euler角
    ros::Subscriber vicon_uav_twist_sub = nh.subscribe<geometry_msgs::TwistStamped>("/vrpn_client_node/UAV_6_2/twist", 10, twist_uav_cb);
	...
	//float error_pos = pose_pen_mocap[0] - pose_uav_mocap[0] - delta;
        float pen_theta = asin(error_pos / 0.548);
	...
	//float theta = theta_0;
        theta_uav = pen_theta + time_delay * theta_dot - uav_theta;
	...
	int mode;
	if((theta * 180/3.14) > -10 && (theta * 180/3.14) < 10)
		mode = 6;
	else
		mode = 5;

	if(theta * theta_uav < 0 && abs(theta_uav * 180/3.14) >= 5)
		theta = theta;
	else
		theta = theta;
