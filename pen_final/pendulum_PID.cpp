...
{
    Eigen::Vector3d twist_uav_mocap_enu(msg->twist.linear.x, msg->twist.linear.y, msg->twist.linear.z);
...
int main(int argc, char **argv)
{
    ...
    // 【订阅】vicon uav velocity
    ros::Subscriber vicon_uav_vel_sub = nh.subscribe<geometry_msgs::TwistStamped>("/vrpn_client_node/UAV_6_2/twist", 10, twist_uav_cb);
	...
	if(times % 3 == 0){
		...
		continue;
            ...
            Command_now.pos_sp[0] = pose_uav_mocap[0];
            Command_now.pos_sp[1] = pose_uav_mocap[1];
            Command_now.pos_sp[2] = pose_uav_mocap[2];
            Command_now.vel_sp[0] = twist_uav_mocap[0];
            Command_now.yaw_sp = 0;
            Command_now.pen_sp = 0 - theta;
            Command_now.pen_vel_sp = 0 - theta_dot;
            Command_now.comid = comid;
            comid++;
